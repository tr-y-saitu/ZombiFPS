...
void BattleRifle::UpdateMove(VECTOR setPosition, Player::State playerState)
{
    // Runステート用の座標
...
VECTOR BattleRifle::FixedRunPosition(Player::State playerState)
{
    // 走りアニメーション時の処理
...
VECTOR BattleRifle::FixedReloadPosition(Player::State playerState)
{
    // リロードアニメーション時の処理
...
/// 銃を発砲する
/// </summary>
void BattleRifle::UpdateShooting(VECTOR cameraPosition, VECTOR targetPosition)
{
    // 弾丸の初期化用データの更新
...
/// 武器強化時の初期化
/// </summary>
void BattleRifle::InitializePowerUpWeapon()
{
    // 武器強化指示が出た場合
    if (powerUpWeapon)
    {
...
/// 武器強化時のモデルマテリアルの更新
/// </summary>
void BattleRifle::UpdatePowerUpGunMaterial()
{
    // 強化済みか確認
